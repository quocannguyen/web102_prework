...
let unfundedCount = unfunded.length

// create a string that explains the number of unfunded games using the ternary operator
const displayStr = `A total of $${totalPledged.toLocaleString('en-US')} has been raised for ${GAMES_JSON.length} games. Currently, ${unfundedCount} ${unfundedCount != 1 ? "game remains" : "games remain"} unfunded. We need your help to fund these amazing games!`
...
const firstGameCard = document.createElement("div")
firstGameCard.innerHTML = `<p>${firstGame.name}</p>`
...
function scrollToOurGames() {
    document.getElementById("our-games-section").scrollIntoView()
}

document.getElementById("our-games-header").addEventListener("click", scrollToOurGames)
...
function searchGame(input) {
    ...
    let searched = GAMES_JSON.filter((game) => {
        return game.name.toLowerCase().includes(input) || game.description.toLowerCase().includes(input)
    })
    deleteChildElements(gamesContainer);
